...
public class LoggerInterceptor implements Interceptor {
    ...
    private void sendLogRequest(Request request) throws IOException {
        if (request != null) {
            ...
            if (request.body() != null) {
                ...
                body = URLEncoder.encode(body, "UTF-8").replaceAll("%26", "&").replaceAll("%3D", "=");
            }
            /*for (String s : request.headers().names()) {
                System.out.println("name:" + s + "    value:" + request.headers().get(s));
            }*/
    ...
    private void sendLogResponse(Response response) throws IOException {
        String rBody = "";
      //  Log.e("contentType",response.body().contentType().toString());
